...
class DraftListViewController: UITableViewController, StoreSubscriber{
    ...
    func configureBackButton() {
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(title: "Back", style: .done, target: self, action: #selector(self.backToOptions(sender:)))
    }
    
    @objc func backToOptions(sender: AnyObject) {
        ViaStore.sharedStore.dispatch(PopNavigation())
    }
    
    func newState(state: AppState) {
        let newDrafts = state.draft.drafts
        switch newDrafts{
        ...
        DispatchQueue.main.async{
            do{
                try ViaStore.draftCache?.fetch([Post].self){ data in
                    if data != nil{
                        ViaStore.sharedStore.dispatch(LoadedDrafts(drafts: .loaded(data: data!)))
                    }else{
                        ViaStore.sharedStore.dispatch(LoadedDrafts(drafts: .loaded(data: [])))
                    }
                }
            }catch let error {
                print(error)
                ViaStore.sharedStore.dispatch(LoadedDrafts(drafts: .error))
            }
        }
    }
